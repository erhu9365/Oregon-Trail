...
Traveler::Traveler()
{
    ...
    if(choice == "Y" || choice == "y") //if y, user changes date to a different date
    {
        ...
        day = stoi(Sday); //converts the given day into an integer
        if(day >= 1 && day <= 31) //checks if it is within the given number range
        {
            day = day;
        }
        else
        ...
        if(month == "March" || month == "April" || month == "June")
        {
            month = month; //sets the month
        }
        else
        {
            cout << "Invalid choice. Please try again." << endl;
...
{
    materials[index].setAmount(namount);
}
