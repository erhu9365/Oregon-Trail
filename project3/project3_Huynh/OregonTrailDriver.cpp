...
void savedata(Traveler user)
{
    ...
    {
        cout << "Leader's Name: " << user.getPlayer(0).getName() << endl; //saves the player's name to the file
        cout << "Miles Traveled: " << user.getMiles() << endl; //saves miles traveled to the file
        cout << "Food Remaining: " << user.getMaterials(1).getAmount() << endl; //saves food remaining to the file
        cout << "Cash Remaining: " << user.getBalance() << endl; //saves the cash remaining to the file
    }
    else 
    {
...
Traveler store(Traveler user, Traveler mainstore)
{
    ...
    while(true)
    {
        ...
        bill = ((mainstore.getMaterials(0).getAmount() * user.getMaterials(0).getCost()) + (mainstore.getMaterials(1).getAmount() * user.getMaterials(1).getCost())
                + (mainstore.getMaterials(2).getAmount() * user.getMaterials(2).getCost()) + (mainstore.getMaterials(3).getAmount() * user.getMaterials(3).getCost())
        ...
        if(choice == 1)
        {
            ...
            {
                if(isdigit(amount[i]) == false)
                {
                    cout << "Your choice cannot contain characters. Please try again." << endl;
                }
                else
                {
                    NumAmount = stoi(amount);
                    NumAmount = NumAmount + user.getMaterials(0).getAmount();
                }
            }
            ...
            {
                mainstore.setMaterialAmount(NumAmount, 0);
            }
            else if(NumAmount * 40 > user.getBalance())
            {
        ...
        if(choice == 2)
        {
            ...
            for(int i = 0; i < amount.length(); i++)
            {
                ...
                {
                    NumAmount = stoi(amount);
                    NumAmount = NumAmount + user.getMaterials(1).getAmount();
                }
            }
            if(NumAmount * user.getMaterials(1).getCost() < user.getBalance() - bill)
            {
                mainstore.setMaterialAmount(NumAmount, 1);
            }
            else
            {
        ...
        if(choice == 3)
        {
            ...
            for(int i = 0; i < amount.length(); i++)
            {
                ...
                {
                    NumAmount = stoi(amount);
                    NumAmount = NumAmount + user.getMaterials(2).getAmount();
                }
            }
            if(NumAmount * user.getMaterials(2).getCost() < user.getBalance() - bill)
            {
                mainstore.setMaterialAmount(NumAmount * 20, 2);
            }
            else
            {
                cout << "Sorry! Looks like you don't have enough." << endl;
        ...
        if(choice == 4)
        {
            ...
            for(int i = 0; i < amount.length(); i++)
            {
                ...
                {
                    NumAmount = stoi(amount);
                    NumAmount = NumAmount + user.getMaterials(3).getAmount();
                }
            }
            if(NumAmount * user.getMaterials(3).getCost() < user.getBalance() - bill)
            {
                mainstore.setMaterialAmount(NumAmount, 3);
            }
            else
            {
                cout << "Sorry! Looks like you don't have enough." << endl;
        ...
        for(int i = 0; i < amount.length(); i++)
        {
            ...
            {
                NumAmount = stoi(amount);
                NumAmount = NumAmount + user.getMaterials(4).getAmount();
            }
        }
            if(NumAmount * user.getMaterials(4).getCost() < user.getBalance() - bill)
            {
                mainstore.setMaterialAmount(NumAmount, 4);
            }
            else
            {
                cout << "Sorry! Looks like you don't have enough." << endl;
        ...
        else if(choice == 0) 
        {
            for(int i = 0; i < 7; i++)
            {
                user.setMaterialAmount(mainstore.getMaterials(i).getAmount() + user.getMaterials(i).getAmount(), i);
            }
...
Traveler raiderattack(Traveler user)
{
    ...
    cin >> choice; //users inputs their choice
    if(choice == 1) //if the player chooses to run
    {
        system("clear"); //clears terminal
        cout << "Your party escaped the raiders, but at what cost?" << endl;
        user.setMaterialAmount(user.getMaterials(0).getAmount() - 1, 0); //decreases the oxen amount by 1 from the player's inventory
        user.setMaterialAmount(user.getMaterials(1).getAmount() - 10, 1); //decreases the food amount by 10 from the player's inventory
        user.setMaterialAmount(user.getMaterials(3).getAmount() - 1, 3); //decreases the wagon parts amount by 1 from the player's inventory
    ...
    else if(choice == 2) //if the player chooses to attack
    {
        ...
        if(fightchance == true) //if the player wins
        {
            ...
            user.setMaterialAmount(user.getMaterials(1).getAmount() + 50, 1); //increases the food amount by 50 from the player's inventory
            user.setMaterialAmount(user.getMaterials(2).getAmount() + 50, 2); //increases the bullets amount by 50 from the player's inventory
            clear(); //clears terminal
        }
        else //if the player loses
        {
            cout << "After fighting with the raiders, you and your party suffered great losses." << endl;
            user.setMaterialAmount(user.getMaterials(2).getAmount() - 50, 2);//decreases the bullet amount by 50 from the player's inventory
    ...
    else //if the player inputs something that is not in the given options
    {
        ...
        cin >> choice; //sends user back to enter an input again
    }
...
Traveler misfortune(Traveler user)
{
    ...
    if(event == 1) //if the event is chosen to be a illness
    {
        ...
        if(sickness == 1) //the chosen person is given typhoid
        {
            ...
            else //if they don't have any medkits
            {
                ...
                cin >> choice; //user inputs to either rest or continue
                ...
                else //if the player inputs something that is not in the given options
                {
                    ...
                    cin >> choice; //returns the user back to input a choice
                }
            }
        }
    ...
    else if(event == 3) //the player's wagon breaks down
    {
        ...
        user.setMaterialAmount(user.getMaterials(3).getAmount() - 1, 3); //decreases the wagon parts amount by 1 from the player's inventory to fix it
...
Traveler stop(Traveler user)
{
    ...
    if(restdays == 1||restdays == 2||restdays == 3)
    {
        user.setTimeDay(restdays);
        for(int i = 0; i < 5; i++)
        {
            if(user.getPlayer(i).getStatus() == true)
            {
                user.setMaterialAmount(user.getMaterials(1).getAmount() - (3 * restdays), 1);
            }
        }
    }
    return user;
...
Traveler continueon(Traveler user, Traveler mainstore)
{
    ...
    for(int i = 0; i < 5; i++) //loops through the party
    {
        ...
        {
            user.setMaterialAmount(user.getMaterials(1).getAmount() - (3 * 14), 1); //decreases the food amount by 3 * 14 for each person from the player's inventory
        }
    }
    ...
    if(user.getMiles() >= user.getCheckpoint().getMiles()) //checks if the user is at a milestone
    {
        ...
        {
            int choice;
            cin >> choice; //user inputs either option
            ...
            while(true) //loops until the user inputs a valid choice
            {
                ...
                {
                    cout << "Invalid choice. Please try again." << endl;
                    cin >> choice; //the user inputs another choice
                }
            }
        }
        ...
        {
            int choice;
            cin >> choice; //user inputs either of the three options
            cout << "What do you want to do?:" << endl;
            cout << "1. Rest" << endl;
            cout << "2. Continue" << endl;
        ...
        {
            int choice;
            cin >> choice; //the user inputs either option
            cout << "What do you want to do?:" << endl;
            cout << "1. Rest" << endl;
            cout << "2. Continue" << endl;
            while(true) //loops until the user inputs a valid choice
...
Traveler hunt(Traveler user)
{
    ...
    if(randnumbers(1, 2) == 1)
    {
        ...
        if(choice == "Y")
        {
            if(user.getMaterials(2).getAmount() >= 10)
            {
                if(puzzle() == true)
                {
                    cout << "Your hunt was a success! You have gained 2 lbs of food." << endl;
                    retrivedfood = retrivedfood + 2;
                    user.setMaterialAmount(user.getMaterials(2).getAmount() - 10, 2);
                }
                else
                {
        ...
        if(choice == "Y")
        {
            if(user.getMaterials(2).getAmount() >= 10)
            {
                if(puzzle() == true)
                {
                    ...
                    retrivedfood = retrivedfood + 8;
                    user.setMaterialAmount(user.getMaterials(2).getAmount() - 8, 2);
                }
                else
                {
                    cout << "The hunt was unsuccessful." << endl;
    ...
    int deerchance = randnumbers(1, 100);
    if(deerchance >= 1 && deerchance <= 15)
    {
        string choice;
        ...
        if(choice == "Y")
        {
            if(user.getMaterials(2).getAmount() >= 10)
            {
                if(puzzle() == true)
                {
                    ...
                    retrivedfood = retrivedfood + 60;
                    user.setMaterialAmount(user.getMaterials(2).getAmount() - 5, 2);
                }
                else
                {
                    cout << "The hunt was unsuccessful." << endl;
    ...
    int bearchance = randnumbers(1, 100);
    if(bearchance >= 1 && bearchance <= 7)
    {
        string choice;
        ...
        if(choice == "Y")
        {
            if(user.getMaterials(2).getAmount() >= 10)
            {
                if(puzzle() == true)
                {
                    ...
                    retrivedfood =  retrivedfood + 500;
                    user.setMaterialAmount(user.getMaterials(2).getAmount() - 12, 2);
                }
                else
                {
                    cout << "The hunt was unsuccessful." << endl;
        ...
        while(true)
        {
            if(choice == "P")
            {
                for(int i = 0; i < 5; i++)
                {
                    if(user.getPlayer(i).getStatus() == true)
                    {
                        user.setMaterialAmount(user.getMaterials(1).getAmount() - 2, 1);
                    }
                }
                break;
            }
            else if(choice == "M")
            {
                for(int i = 0; i < 5; i++)
                {
                    if(user.getPlayer(i).getStatus() == true)
                    {
                        user.setMaterialAmount(user.getMaterials(1).getAmount() - 3, 1);
                    }
                }
                break;
            }
            else if(choice == "W")
            {
                for(int i = 0; i < 5; i++)
                {
                    if(user.getPlayer(i).getStatus() == true)
                    {
                        user.setMaterialAmount(user.getMaterials(1).getAmount() - 5, 1);
                    }
                }
                break;
            }
    ...
    if(user.getMaterials(1).getAmount() + retrivedfood <= 1000)
    {
        user.setMaterialAmount(user.getMaterials(1).getAmount() + retrivedfood, 1);
    }
    else
    {
        cout << "The wagon can only hold 1000 lbs of food. Some of it had to be left behind." << endl;
        user.setMaterialAmount(1000, 1);
        clear();
    }
    user.setTimeDay(1);
    return user;
...
Traveler usermenu(Traveler user, Traveler mainstore)
{
    ...
    while(true) //loops the menu until a break statement
    {
        ...
        cout << "Next Landmark: " << user.getCheckpoint().getMiles() << " miles" << endl;
        ...
        else //if the user inputs something not in the given options
        {
            cout << "Invalid choice. Please try again." << endl;
            clear();
        }
...
int main()
{
    ...
    while(user.getMiles() < 2048) //while the player has not reach the end which is Oregon City
    {
        ...
        user.getMaterials(1).getAmount() <= 0||user.getMaterials(3).getAmount()||
